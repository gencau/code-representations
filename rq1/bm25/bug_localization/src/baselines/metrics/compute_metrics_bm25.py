...
def calculateResults(column: str,
    ...
    df[f'{column_prefix}Acc@k{column_suffix}'] = df.apply(
        lambda r: metrics.acc_at_k(r['changed_files'],
                                   preds(r),
                                   k=k(r)),
        axis=1
    )
    ...
    all_acck = df[column_prefix+'Acc@k'+column_suffix].dropna().mean()
    ...
    print(f"Overall Acc@k {column_suffix}: {all_acck}")
...
def computeMetrics(outfile: str, percent: int, topk: int):
    ...
    if percent > 0:
        ...
        df['top_k'] = df['final_files'].apply(lambda lst: topk)
    else:
