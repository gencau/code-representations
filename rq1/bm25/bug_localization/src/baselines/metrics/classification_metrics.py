...
def acc_at_k(gt_locations, predicted_locations, k=1):
    """
    Acc@k: 1.0 if ALL ground-truth locations are within the top-k predictions; else 0.0.
    """
    if not gt_locations:
        # Convention: if no GT, return 0.0 (or skip this example in your aggregate)
        return 0.0
    gt : set = set(gt_locations)
    return 1.0 if gt.issubset(predicted_locations[:k]) else 0.0
