...
def count_python_sources(path: str):
    ...
    # If repo_file_counts.csv exists, read it and see if we have already counted this repo
    counts_file = Path("repo_file_counts.csv")
    if counts_file.exists():
        with open(counts_file, "r") as f:
            for line in f:
                repo, count = line.strip().split(",")
                if repo == str(repo_path):
                    print(f"Found existing count for {repo_path}: {count} files")
                    return int(count)
    ...
    # Cache the count so we only have to do this once.
    # Write file with repo_path and num_files, comma-separated
    with open("repo_file_counts.csv", "a") as f:
        f.write(f"{repo_path},{num_files}\n")
